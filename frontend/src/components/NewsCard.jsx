...
const NewsCard = ({ news, variant = "default" }) => {
  ...
  const uploadTime = news.createdAt
  ...
  if (variant === "horizontal") {
    return (
      <div className="flex gap-4 group">
        <Link
          to={`/news/${news.slug}`}
          className="shrink-0 w-28 h-20 md:w-36 md:h-24 overflow-hidden rounded-sm"
        >
          <img
    ...
    <article className="group card-shadow bg-white/50 dark:bg-ink-soft/50 rounded-sm overflow-hidden flex flex-col h-full">
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-center justify-between mb-2">
          <span className="dateline text-crimson">
            {news.category?.name}
          </span>
          ...
          <button
            ...
            className={
              bookmarked
                ? "text-gold"
                : "text-ink/30 dark:text-paper/30 hover:text-gold"
            }
          >
            <FiBookmark
              size={16}
        ...
        <p className="text-sm text-slate-650 dark:text-paper-dim/70 line-clamp-2 mb-3">
          {news.description}
        </p>
